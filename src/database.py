...
class Supabase:
    ...
    def get_player_by_id(self, id: int) -> dict | None:
        ...
        response = (
            self.client.table(common.PLAYER_TABLE).select("*").eq("id", id).execute()
        )
    ...
    def update_player(self, id: int, codename: str):
        ...
        self.client.table(common.PLAYER_TABLE).update({"codename": codename}).eq(
            "id", id
        ).execute()
    ...
    def create_player(self, id: int, codename: str):
        ...
        self.client.table(common.PLAYER_TABLE).insert(
            {"id": id, "codename": codename}
        ).execute()